...
import { SimplePMHTextarea } from './SimplePMHTextarea';
...
export function ImprovedPMHSection({ data, onChange }: ImprovedPMHSectionProps) {
  ...
  // Initialize with empty entries if needed - only on mount
  useEffect(() => {
  ...
  const updateMainCondition = useCallback((entryId: string, value: string) => {
    // 🔍 DEBUG: Track callback calls and data.entries identity  
    console.log('📤 updateMainCondition called:', {
      entryId,
      value,
      dataEntriesId: data.entries.map(e => ({ id: e.id, condition: e.mainCondition })),
      dataEntriesIdentity: data.entries,
      callbackIdentity: updateMainCondition,
    ...
    console.log('🔄 Calling parent onChange - this will trigger re-render');
    onChange({ entries: updatedEntries });
  }, [data.entries, onChange]);
