...
import { phraseCache, apiResponseCache } from '../lib/intelligentCache'
import { debounce } from '../electron/performanceManager'
...
export function useOptimizedDotPhrases() {
  ...
  const debouncedCreateDotPhrase = useCallback(
    debounce(async (phraseData: Omit<DotPhrase, 'id'>): Promise<DotPhrase> => {
      if (!isAuthenticated || !idToken) {
  ...
  const debouncedUpdateDotPhrase = useCallback(
    debounce(async ({ id, ...phraseData }: DotPhrase): Promise<DotPhrase> => {
  ...
  const createMutation = useMutation({
    mutationFn: debouncedCreateDotPhrase,
    onMutate: async (newPhrase) => {
      // Optimistic update
      await queryClient.cancelQueries({ queryKey: ['dotPhrases', idToken] })
      const previousPhrases = queryClient.getQueryData(['dotPhrases', idToken])
      
      queryClient.setQueryData(['dotPhrases', idToken], (old: DotPhrase[] = []) => [
        ...old,
        { ...newPhrase, id: Date.now() } // Temporary ID
      ])
      
      return { previousPhrases }
    },
    onError: (err, newPhrase, context) => {
      // Rollback on error
      if (context?.previousPhrases) {
        queryClient.setQueryData(['dotPhrases', idToken], context.previousPhrases)
      }
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: debouncedUpdateDotPhrase,
    onMutate: async (updatedPhrase) => {
      await queryClient.cancelQueries({ queryKey: ['dotPhrases', idToken] })
      const previousPhrases = queryClient.getQueryData(['dotPhrases', idToken])
      
      queryClient.setQueryData(['dotPhrases', idToken], (old: DotPhrase[] = []) =>
        old.map(phrase => phrase.id === updatedPhrase.id ? updatedPhrase : phrase)
      )
      
      return { previousPhrases }
    },
    onError: (err, updatedPhrase, context) => {
      if (context?.previousPhrases) {
        queryClient.setQueryData(['dotPhrases', idToken], context.previousPhrases)
      }
    },
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (deletedId) => {
      ...
      queryClient.setQueryData(['dotPhrases', idToken], (old: DotPhrase[] = []) =>
        old.filter(phrase => phrase.id !== deletedId)
      )
