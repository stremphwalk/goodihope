...
export function DotPhraseManager({ onDotPhrasesChange }: DotPhraseManagerProps) {
  ...
  const form = useForm({
    resolver: zodResolver(dotPhraseSchema),
    defaultValues: {
      trigger: '',
      content: '',
      description: '',
  ...
  useEffect(() => {
    if (editingId && customPhrases.find(p => p.id === editingId)) {
      const phrase = customPhrases.find(p => p.id === editingId);
      if (phrase) {
        form.setValue('trigger', phrase.trigger);
        form.setValue('content', phrase.content);
        form.setValue('description', phrase.description);
        form.setValue('category', phrase.category);
      }
    }
  }, [editingId, customPhrases, form]);
  ...
  const handleSmartFunctionInsert = (content: string) => {
    ...
    if (!textarea) {
      ...
      form.setValue('content', currentContent + content);
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        await updateMutation.mutateAsync({ id: editingId, phrase: form.getValues() });
        setEditingId(null);
      } else {
        // Create new phrase
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="/myshortcut" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder={t('dotManager.description')} {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                      ...
                      <div className="p-3 bg-white border-t">
                        <div className="text-xs text-gray-600 space-y-1">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-medium">{parseSmartOptions(form.watch('content')).length}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span>Character Length:</span>
                            <span className="font-medium">{form.watch('content').length}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span>Contains Calculations:</span>
                            <span className="font-medium">{form.watch('content').includes('/calc') ? 'Yes' : 'No'}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span>Contains Dates:</span>
                            <span className="font-medium">{form.watch('content').includes('[[DATE]]') ? 'Yes' : 'No'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
